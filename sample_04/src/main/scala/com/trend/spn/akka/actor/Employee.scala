...
object Employee {
  def props(name: String, age: Int): Props =
    Props(classOf[Employee], name, age).withDispatcher("prio-dispatcher")
}
...
class Employee(name: String, age: Int) extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    case Gossip => {
      println(s"$myname($myage) is talking")
      Thread.sleep(2000)
    }
    case Nap => {
      println(s"$myname($myage) is going to take a nap")
      Thread.sleep(5000)
    }
    case Meeting => {
      println(s"$myname($myage) is in meeting")
      Thread.sleep(3000)
    }
  }
